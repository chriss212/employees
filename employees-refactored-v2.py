...
class TransactionHistory:
    """Manages transaction history for employees."""
    
    def __init__(self):
        self._history: Dict[str, List[TransactionRecord]] = {}
    
    def add_transaction(self, employee_name: str, operation_type: str, 
                       amount: float, description: str) -> None:
        """Add a transaction to employee history."""
        if employee_name not in self._history:
            self._history[employee_name] = []
        ...
        self._history[employee_name].append(record)
    
    def get_history(self, employee_name: str) -> List[TransactionRecord]:
        """Get transaction history for employee, ordered by date descending."""
        history = self._history.get(employee_name, [])
...
class SalariedPayrollCalculator:
    def __init__(self, policy_manager: PaymentPolicyManager, history: TransactionHistory):
    ...
    def calculate_pay(self, employee: SalariedEmployee) -> float:
        ...
        self.history.add_transaction(
            ...
            "payment", 
            total_pay, 
            f"Monthly salary payment including bonuses"
        )
...
class HourlyPayrollCalculator:
    ...
    def calculate_pay(self, employee: HourlyEmployee) -> float:
        ...
        self.history.add_transaction(
            ...
            "payment", 
            total_pay, 
            f"Hourly payment: {employee.hours_worked} hours"
        )
...
class FreelancerPayrollCalculator:
    ...
    def calculate_pay(self, employee: FreelancerEmployee) -> float:
        ...
        self.history.add_transaction(
            ...
            "payment", 
            total_pay, 
            f"Project payment with bonuses"
        )
    ...
    def take_vacation(self, employee: Employee, payout: bool, days: int) -> None:
        ...
        if payout:
            ...
            self.history.add_transaction(
                employee.name, 
                "vacation_payout", 
                config.FIXED_VACATION_DAYS_PAYOUT, 
                f"Vacation payout: {config.FIXED_VACATION_DAYS_PAYOUT} days"
            )
        else:
...
class PayrollService:
    def __init__(self, repository: EmployeeRepository, ui: UIRenderer, 
                 policy_manager: PaymentPolicyManager, history: TransactionHistory):
        self._repository = repository
        self._ui = ui
        self._policy_manager = policy_manager
        self._calculator_registry = PayrollCalculatorRegistry(policy_manager, history)
        self._setup_calculators()
    
    def _setup_calculators(self) -> None:
        """Setup the calculator registry with all employee types."""
        self._calculator_registry.register(SalariedEmployee, SalariedPayrollCalculator(self._policy_manager, self._calculator_registry.history))
        self._calculator_registry.register(HourlyEmployee, HourlyPayrollCalculator(self._policy_manager, self._calculator_registry.history))
        self._calculator_registry.register(FreelancerEmployee, FreelancerPayrollCalculator(self._policy_manager, self._calculator_registry.history))
...
class EmployeeManagementApp:
    def __init__(self, repo: EmployeeRepository, ui: UIRenderer, vacation_manager: VacationManager):
        ...
        self.history = TransactionHistory()  # Inject history dependency
        self.payroll_service = PayrollService(repo, ui, self.policy_manager, self.history)
    ...
    def _view_transaction_history(self):
        ...
        try:
            ...
            emp = employees[idx]
            history = self.history.get_history(emp.name)
            
            if not history:
                self.ui.display_message(f"No transaction history for {emp.name}.")
                return
            
            self.ui.display_message(f"\nTransaction History for {emp.name}:")
            self.ui.display_message("=" * 50)
            
            for record in history:
                self.ui.display_message(
                    f"{record.date} | {record.operation_type} | "
                    f"${record.amount:.2f} | {record.description}"
                )
...
def main():
    ...
    vacation_manager = StandardVacationManager(history)
