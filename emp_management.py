...
# ROLE-BASED VACATION STRATEGIES

class InternVacationManager:
    """Vacation manager for interns - no vacation allowed."""
    
    def take_vacation(self, employee: Employee, payout: bool, days: int = 1) -> None:
        raise ValueError("Interns are not allowed to take vacation or receive payout")


class ManagerVacationManager:
    """Vacation manager for managers - can take up to 10 days payout."""
    
    def take_vacation(self, employee: Employee, payout: bool, days: int = 1) -> None:
        if payout:
            self._handle_payout(employee, days)
        else:
            self._handle_time_off(employee, days)
    
    def _handle_payout(self, employee: Employee, days: int) -> None:
        max_payout_days = 10
        if days > max_payout_days:
            raise ValueError(f"Managers can only get payout for up to {max_payout_days} days per request. Requested: {days}")
        if employee.vacation_days < days:
            raise ValueError(f"Insufficient vacation days. Available: {employee.vacation_days}, Requested: {days}")
        
        employee.reduce_vacation_days(days)
        print(f"Paying out {days} vacation days. Holidays left: {employee.vacation_days}")
    ...
    def _handle_time_off(self, employee: Employee, days: int) -> None:
        ...
        employee.reduce_vacation_days(days)
        print(f"Have fun on your {days}-day holiday. Don't forget to check your emails!")


class VicePresidentVacationManager:
    """Vacation manager for vice presidents - unlimited vacation, max 5 days per request."""
    ...
    def _handle_payout(self, employee: Employee, days: int) -> None:
        ...
        if days > max_days_per_request:
            raise ValueError(f"Vice presidents can take maximum {max_days_per_request} days per request. Requested: {days}")
        if employee.vacation_days < days:
            raise ValueError(f"Insufficient vacation days. Available: {employee.vacation_days}, Requested: {days}")
    ...
    def _handle_time_off(self, employee: Employee, days: int) -> None:
        ...
        employee.reduce_vacation_days(days)
        print(f"Enjoying {days} days off. As a VP, you have unlimited vacation!")


class RoleBasedVacationManager:
    """Manager that delegates vacation handling to role-specific strategies."""
    
    def __init__(self):
        self._strategies = {
            EmployeeRole.INTERN: InternVacationManager(),
            EmployeeRole.MANAGER: ManagerVacationManager(),
            EmployeeRole.VICE_PRESIDENT: VicePresidentVacationManager()
        }
    
    def take_vacation(self, employee: Employee, payout: bool, days: int = 1) -> None:
        strategy = self._strategies.get(employee.role)
        if not strategy:
            raise ValueError(f"No vacation strategy defined for role: {employee.role}")
        
        strategy.take_vacation(employee, payout, days)
...
class GrantVacationCommand(Command):
    ...
    def __init__(self, vacation_manager: VacationManager, ui: UIRenderer,
                 employee: Employee, payout: bool, days: int = 1):
        ...
        try:
            ...
            if 0 <= idx < len(employees):
                ...
                if selected_employee.role == EmployeeRole.INTERN:
                    ...
                    days_input = self._ui.get_input(f"How many days? {limit_msg}: ")
                    days = int(days_input)
                
                command = GrantVacationCommand(self._vacation_manager, self._ui,
                                             selected_employee, payout, days)
                command.execute()
            else:
...
def create_app() -> EmployeeManagementApp:
    ...
    vacation_manager = RoleBasedVacationManager()
